...
def check_PAR(sv,gen):
	in_par = False
	if len(BedTool(sv,from_string=True).intersect(BedTool(get_path()+'/resources/par_'+gen+'.bed'),f=0.5)) > 0: in_par=True
	return in_par
def dump_json(o,d):
	ofh=open(o,'w')
...
def mask_bed(fh,gen):
	return BedTool(fh).subtract(get_path()+'/resources/{}_excluded.bed.gz'.format(gen))
